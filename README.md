...
...터미널에 원격으로 접근할 방법이 필요합니다. Claude Web Terminal은 ~11MB 단일 바이너리로, 외부 의존성 없이 이 모든 걸 제공합니다.
...
```
 폰 / 태블릿 / 데스크톱
          ...
          ▼
 ┌──────────────────────┐
 │   claude-terminal    │  Go 단일 바이너리 (~11MB)
 │                      │
 │  ┌────────────────┐  │
...
- **TOTP 2FA** — Google Authenticator 등 TOTP 앱 지원, 복구 코드 5개 제공
- **중복 로그인 방지** — 동일 IP에서 새 세션 생성 시 기존 세션 자동 만료
...
├── totp.go              # TOTP 2FA (순수 Go 구현, 외부 의존성 없음)
├── asset_server.go      # 런타임 에셋 최적화 (미니파이 + gzip + ETag)
...
빌드 도구 없이 — `go build` 한 방이면 최적화된 바이너리가 완성됩니다.

- **런타임 에셋 최적화** — 서버 시작 시 모든 정적 파일을 자동으로 미니파이(tdewolff/minify) + gzip 압축 후 메모리에 캐싱
- **JS/CSS 전송량 83% 감소** — 원본 330KB → gzip 전송 ~55KB
- **ETag 기반 304 응답** — 재방문 시 변경 없으면 0바이트 전송
- **스마트 캐싱** — JS/CSS는 7일 캐싱(`?v=N`으로 무효화), HTML은 ETag 재검증
- **외부 빌드 도구 불필요** — webpack, vite, esbuild 없이 순수 Go 런타임에서 처리
